...
export type StartupDependencies = {
  ...
  openRemoteManager: () => void;
  /** Run any pi-desktop:// links queued before boot finished. */
  drainPendingDeepLinks: () => Promise<void>;
...
export function registerApplicationStartup(deps: StartupDependencies): void {
  ...
  void app.whenReady().then(async () => {
    ...
    await ensureWindow();
    // pi-desktop:// links queued before boot (argv on launch, OS events) run
    // once the window exists so their dialogs have a parent.
    if (!bootError) await drainPendingDeepLinks();
