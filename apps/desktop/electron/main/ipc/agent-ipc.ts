...
export function registerAgentIpc({
  ...
  handle(IPC.invoke.plansPending, async (input: { sessionId?: string } = {}) => {
    ...
    // Remote plans are pending in their own host; without this merge the
    // plans UI would only ever show local proposals.
    const remote = (await remoteManager?.pendingPlans()) ?? [];
    return { plans: [...(local.plans ?? []), ...remote] };
  });
