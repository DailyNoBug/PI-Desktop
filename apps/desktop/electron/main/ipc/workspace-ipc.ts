...
export function registerWorkspaceIpc({
  ...
  handle(IPC.invoke.projectGet, async () => {
    const remoteWorkspace = getRemoteManager?.()?.getProject();
  ...
  handle(IPC.invoke.projectList, async () => {
    if (!host) throw new Error("host unavailable");
    const local = (await host.call("projects.list")) as {
      projects?: ProjectRecord[];
    };
    const manager = getRemoteManager?.();
    const remote = manager
      ? manager.listProjects().map((project) => {
  ...
  handleWithEvent(IPC.invoke.composerPickFiles, async (event) => {
    if (getRemoteManager?.()?.getProject()) {
  ...
  handle(
    ...
    async (input: { ref?: string; mimeType?: string } = {}) => {
      const remoteManager = getRemoteManager?.();
      if (remoteManager?.getProject()) {
        const result = await remoteManager.readWorkspace(
          null,
        ...
        return {
          kind: result.kind === "tooLarge" ? "tooLarge" : "notImage",
