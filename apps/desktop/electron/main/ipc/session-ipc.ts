...
export function registerSessionIpc({
  ...
  handle(IPC.invoke.sessionList, async () => {
    ...
    const remoteSessionsRaw = await getRemoteManager?.()?.listSessions().catch(() => []) ?? [];
    const remoteSessions = remoteSessionsRaw.map((session) => {
      const provider = session.providerId
        ? providers.find((candidate: { id: string }) => candidate.id === session.providerId)
        : undefined;
      return {
        ...session,
        ...(provider
  ...
  handle(IPC.invoke.sessionCreate, async (input = {}) => {
    ...
    if (input.projectPath && manager?.isRemotePath(input.projectPath)) {
  ...
  handle(
    ...
    async (
      ...
      if (remoteManager?.knowsSession(sessionId)) {
        return {
          session: await remoteManager.forkSession(
            sessionId,
            input.title,
