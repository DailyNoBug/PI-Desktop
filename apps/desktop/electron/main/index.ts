...
function queueDeepLink(input: string): void {
  ...
  if (!link) return;
  if (!remoteManager || !applicationLifecycleState.applicationBooted) {
...
/**
 * Open the remote SSH supervisor right after IPC registration: the handlers
 * read the manager through a getter, and the instance must exist before the
 * first deep link or renderer query arrives.
 */
function openRemoteManager(): void {
...
// pi-desktop:// links arrive through a second launch (Windows/Linux) or
// open-url (macOS). The upstream activation listener only restores the window;
// this one additionally queues the link for the remote SSH deep-link flow.
