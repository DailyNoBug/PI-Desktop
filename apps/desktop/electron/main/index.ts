 ...
 * so the reserved name keeps the prefix and is intercepted before the plugin
 * catalog lookup. The schema carries no credential material; values are
 * applied only inside Main during execution.
 */
const REMOTE_SSH_TOOL_NAME = "plugin_desktop_ssh";
  ...
  return {
    ...
    sidecarParams: {
      ...
      pluginTools: [
        ...
        // The degraded pure-SSH path for remote connections rides on local
        // sessions, so the desktop SSH tool joins the catalog whenever any
        // connection is connected.
        ...(remoteManager?.hasSshExecTargets() ? [remoteSshTool] : []),
  ...
  h.onNotification((method, params) => {
    ...
    if (method === "permissions.request") {
      ...
      void (async () => {
        ...
        } else if (q.toolName === REMOTE_SSH_TOOL_NAME) {
          // First-party tool: executes inside Main against the connection's
          // stored credentials; the agent only ever sees command output.
          try {
            const result = await remoteManager?.sshExec(
              (q.args ?? {}) as {
                action?: string;
                connection?: string;
                command?: string;
            ...
            payload = {
              executionId: q.executionId,
              ok: false,
              errorCode: (e as { errorCode?: string })?.errorCode ?? "TOOL_FAILED",
              content: { error: e instanceof Error ? e.message : String(e) },
            };
          }
        } else if (!tool) {
