...
export function Sidebar({
  ...
  const renderProjectGroup = (entry: ProjectEntry) => {
    ...
    return (
      <section
        key={entry.key}
        className={`sidebar-session-group project-group ${entry.active ? "active" : ""} ${entry.meta.archived ? "archived" : ""} ${draggingProjectKey === entry.key ? "is-dragging" : ""} ${dropTargetProjectKey === entry.key ? "is-drop-target" : ""}`}
        aria-labelledby={projectId}
        data-sidebar-project-group={entry.key}
        onDragOver={(event) => handleProjectDragOver(event, entry.key)}
        onDragLeave={(event) => {
          if (!event.currentTarget.contains(event.relatedTarget as Node | null)) {
            setDropTargetProjectKey(null);
          }
        }}
        onDrop={(event) => handleProjectDrop(event, entry.key)}
      >
        <div
