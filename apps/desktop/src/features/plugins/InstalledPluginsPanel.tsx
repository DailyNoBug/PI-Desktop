...
import { AGENT_PLUGIN_CAPABILITIES, GROUP_LABEL_KEYS, TEMPLATE_IDS } from "./model";
                ...
                <section key={group.id} className="plugins-group">
                  ...
                  >
                    {group.rows.map((plugin) => {
                      ...
                      // A remote workspace has no local agent host, so a
                      // plugin that only acts through the agent is labeled
                      // instead of being silently assumed to work.
                      const agentCapabilityUnavailable =
                        remoteProject &&
