...
/**
 * Capabilities that only take effect while the host's agent runs beside the
 * project. A remote (SSH) workspace has no local agent, so an installed plugin
 * carrying any of these is labeled rather than silently assumed to work.
 */
export const AGENT_PLUGIN_CAPABILITIES: readonly PluginCapability[] = [
  "tools",
  "skills",
  "mcp",
  "agentExtension",
