...
- 面板在固定客户区内打开，不要求正的原生预留宽度；即使 MainChat 低于
  360px 目标也允许打开。打开后如果窗口收窄或侧边栏展开使主窗格低于
  360px，面板自动折叠并把空间还给 MainChat。
...
- 打开和关闭会让码头的 `width` 和 `flex-basis` 与有界 opacity/transform
  ...
  运动帧之前跳变。即使 MainChat 低于 360px 目标，也允许打开面板。
- 面板打开后，渲染器跟踪测量的主窗格宽度；如果之后本机窗口调整大小或
  侧边栏展开使其低于 360px，面板会自动折叠并把内部空间还给 MainChat。
...
21. 工作面板在固定客户区内打开和折叠；分隔线在 244px–720px 内更新
    ...
    即使 MainChat 低于 360px 目标也允许打开；之后调整大小使主窗格低于
    360px 时，面板会自动折叠。
