...
- shell 启动时没有可见的工作面板。 `Cmd/Ctrl + J` 切换
  活动会话的保留面板上下文，无需创建资源选项卡；再次按下会折叠
  已显示的面板并保留上下文。没有活动会话或“设置”是活动页面时，
  快捷键不执行任何操作。背景工件随后可以创建浏览器或当前范围内的
  插件视图，但不会打开可见面板。
...
- 每个资源都可以从菜单中关闭，并且活动资源有
  直接标题关闭控件。关闭活动资源选择正确
  邻居，然后左边；关闭最后一个选项卡会隐藏面板。唯一的面板级折叠控件
  是视口固定的 shell toggle；它隐藏面板但不删除选项卡。
- 面板在固定客户区内打开，不要求正的原生预留宽度；即使 MainChat 低于
  515px 目标也允许打开。打开后如果窗口收窄或侧边栏展开使主窗格低于
  515px，面板自动折叠并把空间还给 MainChat。
- 提交的分隔符调整大小只更新渲染器中的首选宽度。本机窗口边缘拖动只
  调整窗口并通过回流改变 MainChat，不改变面板宽度（D163，ADR 0151）。
...
- 打开和关闭会让码头的 `width` 和 `flex-basis` 与有界 opacity/transform
  反馈一起动画，因此 MainChat 在现有客户区内持续回流，而不是在第一个
  运动帧之前跳变。即使 MainChat 低于 515px 目标，也允许打开面板。
- 面板打开后，渲染器跟踪测量的主窗格宽度；如果之后本机窗口调整大小或
  侧边栏展开使其低于 515px，面板会自动折叠并把内部空间还给 MainChat。
- 渲染器始终请求本机预留宽度为 0，因此操作系统窗口永不扩展
  (ADR 0151)。打开、折叠和最终关闭仅改变承诺的首选宽度。
- 当面板弹性分配打开时，MainChat 会持续回流；窗口本身不会变化。原生
  窗口和侧边栏调整大小不会夹紧或重写面板。
...
20. 流式消息更新保持在聊天渲染边界内；外壳
    ...
    仅仅因为当前助手消息附加了内容
21. 工作面板在固定客户区内打开和折叠；分隔线在 244px–720px 内更新
    渲染器拥有的面板目标，取消分隔线会恢复之前的宽度（ADR 0151）。
    即使 MainChat 低于 515px 目标也允许打开；之后调整大小使主窗格低于
    515px 时，面板会自动折叠。
