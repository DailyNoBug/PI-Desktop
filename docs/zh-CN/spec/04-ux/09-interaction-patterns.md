...
- **排序**优惠最近更新 (`recent`)、创建日期 (`created`)、
  ...
  至 `recent`。在项目标题上静止按住 400ms，或聚焦标题后按
...
保留的项目组支持项目排序。没有重排手柄。在项目标题上静止按住 400ms 开始拖动，因此短按仍会激活并切换折叠，菜单和嵌套会话行保持原有点击行为。根据指针位于目标上半部还是下半部，放置会插入目标之前或之后，并持久化结果。
...
- 项目标题是重排控件：静止按住 400ms 后开始拖动
- 该延迟前移动超过 8px 会取消按住，因此单击仍会选中并切换折叠
...
16. 项目组可通过长按标题或标题上的 `ArrowUp`/`ArrowDown` 重排；
