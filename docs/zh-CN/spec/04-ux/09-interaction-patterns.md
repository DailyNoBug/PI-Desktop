...
### 1. 8 工作面板条目和资源（D128、D142、D154、D173、D179、D207、D405、D406）
...
- Git 是普通内置插件视图，不是工件触发器：用户显式打开 `pi.git`，其当前状态在
  打开、工作区变化、操作后、手动刷新时刷新，且只有视图可见时每五秒轮询一次。
  Stage、unstage、commit、push 与 pull 在按钮动作后直接执行；discard、切换分支
  或创建并切换先要求宿主拥有的原生确认。
...
- 成绩单卡和复习会消耗活动会话的持续时间
  ...
  激活单例“审阅”选项卡。
- 选择 Git 变更会把 Git 视图扩展到宿主拥有的内缩应用窗口表面，并在变更
  文件树上方打开居中 review 弹窗。该弹窗读取当前 Git 状态，默认以旧/新
  左右分栏展示，不提供回滚，也绝不创建、改写或激活消息持有的 Review
  标签页。变更分区是可折叠目录树；AI 提交操作只填入可编辑消息框。
  既有 Review 快照继续在提交和外部仓库变化后保留。
...
- Enter / Tab 接受突出显示的项目；菜单有高亮项时 Enter 和 Cmd/Ctrl+Enter 都不发送
  （该规则先于回车发送设置）。
  否则保持其行为）。
