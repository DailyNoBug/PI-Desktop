...
- 面板在固定客户区内打开，不要求正的原生预留宽度；聊天表面保持 515px
  CSS 最小宽度。打开面板后窗口或侧边栏调整大小不会自动折叠面板，用户
  通过视口固定的 shell toggle 手动折叠。
...
21. 工作面板在固定客户区内打开和折叠；分隔线在 244px–720px 内更新
    ...
    聊天表面保持 515px CSS 最小宽度；之后调整大小不会自动折叠面板。
