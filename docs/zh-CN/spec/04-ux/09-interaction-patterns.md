...
保留的项目组支持项目排序。只有可见的手柄会启动项目拖动，因此项目激活、菜单和嵌套会话行仍保持原有点击行为。根据指针位于目标上半部还是下半部，放置会插入目标之前或之后，并持久化结果。
...
- 拖动手柄在悬停和键盘聚焦时可见（没有不可见的拖动可供性）
- 在悬停期间放置带有重音边框的目标突出显示
- 使用 Esc 键取消拖动
- 拖动反馈：源上不透明度 0.5，目标上强调轮廓
- 聚焦手柄后按 `ArrowUp`/`ArrowDown` 可移动项目并持久化相同的手动顺序
...
16. 项目组可通过可见拖动手柄或手柄上的 `ArrowUp`/`ArrowDown` 重排；
