...
| 主窗格最小可读宽度 | 360像素 | 打开后的自动折叠阈值；小窗口上可以在低于目标时打开，但面板打开后若继续收窄到 360 像素以下则自动折叠 |
| 工作面板宽度（闭合） | 0像素 | 默认隐藏 |
...
因此聊天宽度仅通过回流而改变。即使 MainChat 低于 360px 目标，也允许
...
- 面板打开时保持已提交的 `244..720px` 宽度（默认 280px），即使当前
  客户区会使 MainChat 低于 360px 目标。
- 打开后，渲染器跟踪测量的主窗格宽度。如果之后本机窗口调整大小或侧边栏
  展开使它低于 360px，面板会自动折叠并把内部空间还给 MainChat。
