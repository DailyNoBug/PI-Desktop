...
| 主窗格最小可读宽度 | 515px | 由 MainPane 弹性项预留；侧边栏和工作面板不能占用该宽度 |
| 工作面板宽度（闭合） | 0像素 | 默认隐藏 |
| 工作面板宽度（打开） | `244px–720px`（新用户默认 360px），固定为已提交宽度 | 面板是占用现有客户端区域的流入列；渲染器拥有分隔符（ADR 0151）；已保存宽度不变 |
| Composer shell 最小 | 〜80像素 | 一行草稿+工具栏填充 |
| Composer 工具栏 | MainChat `≥515px` | 左右控件组保持一行且不收缩；模式/权限标签保持单行并省略显示 |
...
- 工作面板从不参与响应式折叠。它保留了它的
  可见时提交的 `244..720px` 宽度（新用户默认 360px）；已保存宽度不变。
