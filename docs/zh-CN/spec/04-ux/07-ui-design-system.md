...
| 主窗格最小可读宽度 | 515像素 | 打开后的自动折叠阈值；小窗口上可以在低于目标时打开，但面板打开后若继续收窄到 515 像素以下则自动折叠 |
| 工作面板宽度（闭合） | 0像素 | 默认隐藏 |
| 工作面板宽度（打开） | `244px–720px`（默认280px），固定为承诺宽度 | 组合的创建触发器保持内容的完整面板宽度；该面板是流入列，从不扩展操作系统窗口（D154/D163、ADR 0151） |
| Composer shell 最小 | 〜80像素 | 一行草稿+工具栏填充 |
| 输入框草稿高度 | 1–7 行文本 | 自动增长；超出第 7 行的内部滚动 |
| 聊天消息最大宽度 | 720px 助手/560px 用户板 | 防止眼距过度拉伸；用户轮流保持紧凑 |
| 窗口最小宽度 | 1040像素 | 由 Electron 强制执行作为基本聊天 shell 最小值；打开的工作面板会在固定窗口内重新排列聊天，并可能将聊天窗格缩小到 515 像素可读目标以下 |
| 窗户最小高度 | 700像素 | 由 Electron 强制执行 |

开放式工作面板是现有客户区内的固定宽度流入列（ADR 0151）；它会
回流 MainChat，但从不扩展操作系统窗口。渲染器请求的原生保留宽度为 0，
因此聊天宽度仅通过回流而改变。即使 MainChat 低于 515px 目标，也允许
打开面板；打开和折叠不会改变持久化的窗口边界。原生浏览器视图遵循
渲染器测量的面板矩形。持续正常范围是用户的窗口大小。在折叠运动开始
之前，任何本机浏览器预览表面已分离，因为它无法参与渲染器 CSS 动画。
Windows 在有界滑动期间使退出的坞站保持不透明；macOS 和 Linux 保留
淡入淡出和滑动退出。

### 10. 1 响应式折叠

- 面板打开时保持已提交的 `244..720px` 宽度（默认 280px），即使当前
  客户区会使 MainChat 低于 515px 目标。
- 打开后，渲染器跟踪测量的主窗格宽度。如果之后本机窗口调整大小或侧边栏
  展开使它低于 515px，面板会自动折叠并把内部空间还给 MainChat。
- 内部分隔线在渲染器中改变面板宽度；向左移动会从 MainChat 取得更多内部
  空间，向右移动则归还空间。本机窗口边缘只调整固定应用窗口。
- 面板打开和折叠只改变流入 flex 分配，不请求正的原生保留宽度；面板首选
  宽度仍是渲染器本地设置。
