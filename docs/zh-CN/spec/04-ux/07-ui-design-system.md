...
侧边栏页脚：透明实用带，没有分隔符。设置、Activity、
插件和通知图标按钮分组在左侧。build/version 芯片右对齐，
仍是更新检查 / 发布入口。悬停和活动状态使用语义侧边栏表面；
两侧都不添加持久卡片填充。
...
- 路由表面在入场动画结束后不持有堆叠上下文，因此编写在路由页面内的浮层 —— 模态框、抽屉或它们的遮罩 —— 无需调整 `z-index` 即可覆盖标题栏带。在 Windows/Linux 上，渲染器绘制的窗口控件位于渲染器浮层之上。因此路由浮层位于 `z-dialog`（40）：对话框自身弹出的叶子级弹出层（60）或 toast（50）—— 两者均 portal 到 `document.body`，处于同一堆叠上下文 —— 仍绘制在该对话框遮罩之上，并仍可点击。
...
- 舞台管理器（仅 macOS）：主机在折叠时重新声明最小边界（永久看门狗）。该看门狗不在 Windows/Linux 上运行，因此没有任何平台会自行重新调整自己窗口的层级（D447）
