...
| 主窗格最小可读宽度 | 515像素 | 聊天表面的 CSS 最小宽度；固定窗口客户区不足时，工作面板仍保持打开 |
| 工作面板宽度（闭合） | 0像素 | 默认隐藏 |
...
| 窗口最小宽度 | 1040像素 | 由 Electron 强制执行作为基本聊天 shell 最小值；聊天表面保持 515 像素 CSS 最小宽度 |
| 窗户最小高度 | 700像素 | 由 Electron 强制执行 |

开放式工作面板是现有客户区内的固定宽度流入列（ADR 0151）；它会
回流 MainChat，但从不扩展操作系统窗口。渲染器请求的原生保留宽度为 0，
因此聊天宽度仅通过回流而改变。聊天表面保持 515px CSS 最小宽度；打开
面板和原生窗口/侧边栏调整大小不会自动折叠面板。原生浏览器视图遵循
...
- 面板打开时保持已提交的 `244..720px` 宽度（默认 280px），聊天表面保持
  515px CSS 最小宽度。
- 原生窗口和侧边栏调整大小不会自动折叠面板；用户通过 shell 控件手动折叠。
