...
- **提供商**工作室：
  ...
  - 已登录的厂商行带账户徽标，其编辑对话框用该徽标替换 API 密钥字段 ——
    这里没有东西可粘贴
- 子智能体新建/编辑表单使用与 Composer 相同的已配置、可运行模型下拉框，按提供商分组，
  并提供“沿用会话模型”选项；只有明确标记 `availableForSubagents` 的模型绑定会出现在列表中。
  “自定义（provider/model）”选项会打开手动输入框；没有可用于子智能体的模型时直接显示该输入框，
  并提示前往模型设置。若已有 pin 不再配置，编辑时仍保留该选项，避免悄悄丢失。通用或重复的
