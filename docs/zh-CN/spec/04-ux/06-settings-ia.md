...
- 带有图标和简洁平行标签的紧凑导航目录，按以下顺序：
  ...
  9. **连接** — Lucide `Server`（SSH Host 与远程项目）
  10. **导入** — Lucide `Download`（从其他工具引入会话和模型配置）
  11. **项目** — Lucide `Archive`（持久项目索引）
  12. **信息** — Lucide `Info`（版本、日志、更新、开发人员）
...
- 目录仍是保持相同顺序的可搜索扁平列表。为便于扫描，目的地分为四个带标题
  ...
  MCP、子智能体）、“工作区”（连接、导入、项目）和“系统”（信息）。标题使用柔和
...
Token 用量**不是设置目的地**（D335 / D404 / ADR 0230）。展开侧边栏页脚中
设置右侧的 Activity 图标会打开只读的十四天已完成回合摘要，数据来自宿主
持久化历史
...
- **SSH 连接**列出 OpenSSH alias 和手动连接，显示名称、目标、来源、状态、重连次数、最后错误和远端 Host 版本。操作包括连接/断开、打开远程项目、测试、刷新、编辑、移除、同步 provider、删除远端 provider 和复制诊断。
- **添加/编辑连接**使用 OpenSSH 配置/手动分段控件。OpenSSH 需要别名；手动连接需要主机名，可选用户和端口。手动认证使用第二组分段控件，在 SSH agent、密码和 identity file 中三选一。密码字段只写不读，已有值只显示状态不回显；identity 通过 Main 拥有的原生文件选择器选择，只保存路径。表单绝不接受或显示私钥内容。
- **打开远程项目**弹出连接选择、主目录、可编辑绝对路径、有界远端目录列表和 Git 仓库标记。目录浏览由 Host 提供，不使用本地目录选择器。
- Provider 控件在同步或删除前明确显示凭据将写入哪台远程机器。
