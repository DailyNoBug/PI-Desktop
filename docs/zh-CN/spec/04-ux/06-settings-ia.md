...
Token 用量**不是设置目的地**（D335 / D430 / ADR 0273）。展开侧边栏页脚中
...
- **厂商账户**卡片（D237），位于默认值与提供商之间：
  ...
  - “编辑账户”打开的对话框带有账户标签，以及与 AI 服务对话框完全相同的双栏
    ...
    全局默认模型
- **语音听写**卡片（D439 / ADR 0279）：包含 STT 端点（OpenAI 兼容 base
...
- 页面标题只出现一次。连接面板使用安静的“通过 SSH 连接到远程设备”小节标题，Add
  对齐其尾侧；列表行只保留连接/断开。
- **SSH 连接**列出已选 OpenSSH alias 和手动连接，显示名称、目标、来源、状态、重连次数和最后错误。行内只有连接/断开。
- **添加连接**打开弹窗。第一屏按需发现 OpenSSH 具体别名；手动添加使用紧凑表单，接受 `host`、`user@host` 和可选端口。手动认证使用第二组分段控件，在 SSH agent、密码和 identity file 中三选一。密码字段只写不读；identity 通过 Main 拥有的原生文件选择器选择，只保存路径。表单绝不接受或显示私钥内容。
