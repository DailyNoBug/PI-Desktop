...
- **步骤**： 1) 重新启动并检查标题栏和应用程序菜单；确认
  面板开始关闭。按 Cmd/Ctrl+J 并检查空面板标题和上下文菜单，然后再次按下以确认
  面板折叠且没有创建或删除任何选项卡。 2）打开两个不同的文件工件，再次打开相同的第一个文件，
  URL 预览，并准备一条已完成的 Bash 行。 3) 打开 header 的统一
  ...
  最短和最长标签的标题右边缘。 4) 关闭活动的中间和边缘项目并验证邻居选择。
  5) 使用视口固定的工作面板切换控件折叠面板，再触发另一个工件。 6) 在会话 A 中，让面板保持打开状态，并显示多个
...
浏览器处于活动状态。 8) 在有足够工作区域的显示器上，记录MainChat宽度，
  ...
  一边观察整个无框窗户，一边重复折叠。9) 带面板打开，从左右边缘调整本机
  窗口大小；之后重复切换侧边栏。10) 在太窄的工作区域上，即使 MainChat
  低于 360px 也打开面板；然后继续缩小窗口或展开侧边栏，确认主窗格继续
  变窄时面板自动折叠并归还空间。11) 最大化时打开或折叠、进入全屏，然后
  恢复正常。12) 在显示之间移动普通窗口、改变活动显示的工作区域几何形状，
  并在一个不变的工作区域内执行普通动作；包括过渡窗口管理器压缩并重新
  定位外部窗口之前的位置显示更改回调。13) 发送有效和格式错误的预留
  有效负载（包括正值、字符串、布尔值、空值、小数和超出范围的值）。14) 重新启动。
- **预期**：启动时不显示面板或欢迎选择器，也没有应用菜单启动器；
  窗口右上角有 viewport-fixed 工作面板切换控件。Cmd/Ctrl+J 打开活动会话的面板，
  无需创建资源选项卡；再次按下会折叠面板并保留上下文。没有活动会话或“设置”处于
  活动状态时快捷键不会执行任何操作。每个工件自动打开停靠的第三列并创建或
  激活一种资源；文件资源是路径键控且重复的资源
  去重复。打开、折叠和
  关闭动画面板的 width/flex 分配及其有界
  opacity/slide，因此 MainChat 连续重排，无需预动画跳转。
  在最小工作区域上，面板即使让 MainChat 低于 360px 也会打开；只有之后窗口
  收窄或侧边栏展开使测量的主窗格继续低于 360px 时，面板才会自动折叠。
  打开面板、折叠面板或调整分隔线只改变固定客户区内的 MainChat 回流；
  不改变操作系统窗口大小（ADR 0151）。一旦面板打开，一个统一的
  ...
  长度。打开菜单暂时隐藏本机浏览器预览，因此它永远不会被遮挡。唯一的
  折叠控件是窗口右上角的 viewport-fixed shell toggle，而不是工作面板
  内容标题中的控件。
...
指针向下保留起始宽度，移动跟随指针
  ...
  返回到基础宽度。
  重启后仅恢复 `{width}`；每个会话的打开状态、选项卡、活动选项卡和浏览器
  资源重置。打开的面板保持其提交的宽度；原生边缘和侧边栏调整只改变
  MainChat 的内部回流，不改变面板首选宽度或操作系统窗口的面板几何。
  如果面板打开后窗口继续收窄或侧边栏展开，使测量的主窗格低于 360px，
  面板会自动折叠并归还空间；刚打开时低于 360px 不会立即折叠。
  打开、折叠和分隔线提交不会请求正的原生预留宽度。兼容预留 seam 对任何
  有效请求都返回 `{requested: 0, reserved: 0}`；重复目标是幂等的。
  Maximized/fullscreen 保留正常的窗口几何行为；Display/work-area 更改与
  当前可用宽度协调，普通移动不会重新应用几何体。重新启动恢复用户的窗口
  大小（原生预留始终为 0，ADR 0151）。格式错误的预留有效负载失败并显示
  `INVALID_ARGUMENT`，不会被强制转换。被取代的成功请求不能带来陈旧的呈现，
  也不会产生第二次调整大小或位置漂移。
  以前的上下文面板覆盖不再存在。
- **链接规格**：`03-runtime/01-ipc-protocol.md`、`04-ux/01-ui-ia.md`、
  `04-ux/07-ui-design-system.md`、`04-ux/08-component-spec.md`、
  `04-ux/09-interaction-patterns.md`、ADR 0151、ADR 0068、D163、D207
- **接受**：F（坚持），质量
- **里程碑**：M5
...
- 期望项目和会话列表在侧边栏主体内滚动，而无需
  ...
  标头。当工作面板打开时，预期唯一的折叠控制是窗口右上角的视口固定
  shell toggle，而不是工作面板内容标题中的控件；它与主窗格右边缘的分隔线
  独立。
