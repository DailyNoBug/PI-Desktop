...
浏览器处于活动状态。 8) 在有足够工作区域的显示器上，记录MainChat宽度，
  ...
  515px 也打开面板；然后继续缩小窗口或展开侧边栏，确认主窗格继续
...
- **预期**：启动时不显示面板或欢迎选择器，也没有应用菜单启动器；
  ...
  在最小工作区域上，面板即使让 MainChat 低于 515px 也会打开；只有之后窗口
  收窄或侧边栏展开使测量的主窗格继续低于 515px 时，面板才会自动折叠。
...
指针向下保留起始宽度，移动跟随指针
  ...
  如果面板打开后窗口继续收窄或侧边栏展开，使测量的主窗格低于 515px，
  面板会自动折叠并归还空间；刚打开时低于 515px 不会立即折叠。
