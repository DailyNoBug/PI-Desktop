...
- **步骤**：1）打开 Git 视图，检查两个变更分区的目录树和折叠/展开后的文件夹
  计数。2）不通过其他界面展开文件夹，直接选择嵌套的已修改文件。3）确认覆盖层
  直接以左右分栏打开，旧/新行号对齐，旧内容为红色、新内容为绿色，变更导航可用。
...
- **预期**：文件夹与文件行保持对齐并跟随语言。点击文件打开 Git 持有的覆盖层，
  绝不打开消息持有的 Review 或回滚。左右分栏是默认模式，两种主题下红/绿旧新
  内容均可读。AI 使用标记的已就绪默认模型，仅通过公共 `models.list` 与
  `agent.complete` 通道发送有界 scoped Git diff 上下文，并且只填入可编辑提交框。
