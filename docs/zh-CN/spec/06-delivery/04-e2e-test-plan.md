...
- **步骤**：1）打开 Git 视图，检查两个变更分区的目录树和折叠/展开后的文件夹
  计数。2）不通过其他界面展开文件夹，直接选择嵌套的已修改文件。3）确认居中
  弹窗直接覆盖文件树并以左右分栏打开，旧/新行号对齐，旧内容为红色、新内容
  为绿色，变更导航可用。
  4）切换到 unified 再切回，用 Escape 关闭并确认焦点回到同一文件行。5）存在
  staged 变更时点击 AI，编辑生成消息后提交。6）清空索引后，在 unstaged 与
  untracked 变更上点击 AI，确认生成信息反映这些变更。7）强制模型失败并重试；
  再在无模型配置中重试。8）使用过大夹具并确认生成输入保持有界。
- **预期**：文件夹与文件行保持对齐并跟随语言。点击文件打开 Git 持有的弹窗，
  绝不打开消息持有的 Review 或回滚，遮罩后的 Git 标签页仍以变更文件树为主。
