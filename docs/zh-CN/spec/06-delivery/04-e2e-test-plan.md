...
#### E2E-254：内置 Git 工作面板管理当前仓库

- **前置条件**：本地裸 `origin` 含 `main` 分支；克隆 A 与克隆 B 均配置为使用该
  远端且不需要终端提示。克隆 A 包含一个已暂存文件、一个已修改文件、一个重命名
  文件、一个未跟踪文件、一个含大量文件的完全未跟踪目录和一个二进制或过大文件，
  并领先 `origin/main`。克隆 B 有
...
- **预期**：Git 视图使用 `branch` 图标并跟随当前语言。它报告仓库、分支、上游、
  ahead/behind、staged、unstaged、untracked 文件和目录、冲突与截断状态，且日志
  不暴露 diff 输出。未跟踪目录保持一个聚合行，因此即使其中包含数千个条目，
  其他被修改文件仍会列出；该目录可整体 stage 或 discard，但不会打开 review。
  Review 弹窗覆盖宿主拥有的内缩应用窗口表面，支持所有范围和导航，
  绝不创建或激活消息持有的 Review 与回滚。Stage/unstage 更新正确分组；
  discard 与切换分支要求宿主拥有的原生确认，已跟踪文件从 `HEAD` 恢复，
  未跟踪文件移入系统回收站。
...
#### E2E-255：Git 树形 review 与 AI 提交信息生成

- **前置条件**：Git 仓库包含嵌套的已变更目录，其中有 staged 与 unstaged 文本
  文件、至少一个多行修改，并且默认模型已就绪。另有第二个可用模型、可关闭
  全部 provider 的无模型配置，以及可触发提示截断的过大或 51 文件夹具。
- **步骤**：1）打开 Git 视图，检查两个变更分区的目录树和折叠/展开后的文件夹
  计数。2）不通过其他界面展开文件夹，直接选择嵌套的已修改文件。3）确认窗口
  居中的弹窗接近应用窗口大小且不被右侧面板裁剪，以左右分栏打开，旧/新行号
  对齐，旧内容为红色、新内容为绿色，变更导航可用。4）调整窗口大小并确认弹窗
  和原位文件树跟随变化；切换离开 Git，确认扩展的原生视图消失，返回后回到
  停靠文件树。5）切换到 unified 再切回，用 Escape 关闭并确认焦点回到同一
  文件行。6）存在 staged 变更时点击 AI，编辑生成消息后提交。7）清空索引后，
  在 unstaged 与 untracked 变更上点击 AI，确认生成信息反映这些变更。8）强制
  模型失败并重试；再在无模型配置中重试。9）使用过大夹具并确认生成输入保持有界。
- **预期**：文件夹与文件行保持对齐并跟随语言。点击文件打开 Git 持有的弹窗，
  绝不打开消息持有的 Review 或回滚，Git 标签页仍以固定在右侧面板原位的
  变更文件树为主。
  左右分栏是默认模式，两种主题下红/绿旧新内容均可读。AI 使用标记的已就绪
  默认模型。提交组合区把消息框和 AI/Commit 操作行保留在同一个边框内，控件
  高度对齐且按钮文字不换行。AI 仅通过公共 `models.list` 与 `agent.complete`
  通道发送有界
  scoped Git diff 上下文，并且只填入可编辑提交框。
  模型、空输出和无模型失败显示本地化错误且不清空用户草稿；截断提示不超过公共
  补全输入限制。提交仍是独立的显式操作。
- **链接规格**：`07-plugins/03-plugin-api.md`（models 与 agent.complete）、
  `04-ux/08-component-spec.md` §5.2.3、
  `04-ux/09-interaction-patterns.md` §1.8；ADR 0232、ADR 0233、D406、D407
- **验收**：D（插件）、安全、质量
- **里程碑**：M6+
- **状态**：草稿；运行时权限与内置插件源码契约测试覆盖固定通道和有界提示构造；
  浏览器 E2E 未运行
