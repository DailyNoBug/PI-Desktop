...
浏览器处于活动状态。 8) 在有足够工作区域的显示器上，记录MainChat宽度，
  ...
  窗口大小；之后重复切换侧边栏。10) 在太窄的工作区域上打开面板，验证聊天表面
  保持 515px CSS 最小宽度；原生窗口或侧边栏调整大小不会自动折叠打开的面板。
  11) 最大化时打开或折叠、进入全屏，然后
...
- **预期**：启动时不显示面板或欢迎选择器，也没有应用菜单启动器；
  ...
  在最小工作区域上，聊天表面保持 515px CSS 最小宽度；之后窗口或侧边栏调整大小
  不会自动折叠面板，shell toggle 仍是用户控制的折叠操作。
...
指针向下保留起始宽度，移动跟随指针
  ...
  如果面板打开后窗口继续收窄或侧边栏展开，聊天表面仍保持 515px CSS 最小宽度；
  面板不会自动折叠，用户可通过 shell toggle 折叠。
