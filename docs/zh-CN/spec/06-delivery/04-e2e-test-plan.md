...
#### E2E-251：项目组支持拖拽和键盘手动排序

- **前提条件**：侧边栏至少有 3 个项目组，其中包含一个已固定或已归档项目；每个项目都有稳定的 host workspace/path/directory。
- **步骤**：
  1. 聚焦项目组的 grip，将其拖到另一个项目组上方或下方，观察放置目标后释放。
  2. 再次聚焦同一 grip，按 `ArrowUp` 或 `ArrowDown` 各移动一次。
  3. 重启应用并检查项目顺序。
  4. 从重排后的项目打开会话，确认 host workspace、path 和 directory 未改变。
- **预期**：项目组按释放后的顺序显示，手动顺序在重启后保留。Grip 支持键盘排序，`Escape` 可取消拖动，既有固定/归档优先级保持不变；排序不会改变项目的 host workspace、path、directory 或会话排序。
- **链接规格**：`04-ux/08-component-spec.md`、`04-ux/09-interaction-patterns.md`、`03-runtime/04-data-storage.md`、`08-meta/decisions-log.md`（D399）
- **验收**：D（工作区）、F（持久化）、品质
- **里程碑**：M5
- **状态**：源代码契约覆盖（`app-store-sidebar.test.mjs`、`sidebar-preferences.test.mjs`）；渲染桌面旅程为草稿
