...
#### E2E-199：子智能体编辑器提供预设模板和按提供商筛选的模型选择器

- **先决条件**：Agent 模式；设置中至少有一个提供商包含标记为“可用于 AI 委派”的模型，
  并有一个模型关闭该标记。`~/.agents/subagents` 目录为空。
- **步骤**：1) 打开“设置 → 智能体 → 子智能体”，点击“新建子智能体”，确认表单上方有
  ...
  且显示可修改工具的提示；展开高级后轮次上限为 80。4) 展开高级并打开模型选择器，确认每个提供商中标记为
  `availableForSubagents` 的模型都出现，而关闭标记的模型不出现。选择一个模型，确认保存值为
  `vendorKey-or-name/modelId`。5) 选择“自定义（provider/model）…”，输入一个模型 pin 并保存；
  再选择“沿用当前会话的模型”，确认模型值为空。6) 禁用所有委派模型，重新打开并展开高级，确认回退为带有模型设置提示的
  单个文本输入框，并仍能校验手动输入的 `provider/model`。7) 切换到简体中文，确认预设和选择器标签
  已翻译，且没有原始 i18n key。
- **预期**：选择器只显示已配置且明确允许子智能体使用的模型；关闭
  `availableForSubagents` 的模型不能从选择器选择。自定义输入仍可指向未配置的提供商，避免把用户
  锁死。选择预设只覆盖描述、工具、正文和轮次上限，不会悄悄清除模型、推理强度或作用域；选择空白
  开始会清除已选预设的这些模板字段。
