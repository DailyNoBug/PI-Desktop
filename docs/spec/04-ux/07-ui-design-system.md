...
| Main pane minimum readable width | 515px | CSS minimum for the chat surface; the fixed-window work panel may remain open when the available client area is smaller |
...
allowed even when MainChat is below 515px; opening and native resizing do not request
  a positive native reservation or automatically collapse the panel. Before collapse
...
### 10.1 Responsive sizing

- The panel opens at its committed `244..720px` width while the chat surface keeps
  its 515px CSS minimum, even when the fixed client area is smaller.
- Native window and sidebar resizing do not automatically collapse the work panel;
  the shell toggle remains the user-controlled collapse action.
