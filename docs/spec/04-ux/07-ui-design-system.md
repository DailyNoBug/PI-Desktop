...
| Main pane minimum readable width | 515px | Readability threshold for resize collapse; opening may begin below this target on small windows, but a later shrink below it while the panel is open collapses the panel |
...
allowed even when MainChat is below 515px; opening and collapsing do not request
...
- The panel opens at its committed `244..720px` width even when the current
  client area leaves MainChat below 515px.
- After opening, the renderer tracks the measured main-pane width. If a later
  native window resize or sidebar expansion makes it narrower than 515px, the
