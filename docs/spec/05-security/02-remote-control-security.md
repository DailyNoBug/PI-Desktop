...
- The desktop presents it once as the WebSocket bearer token over the forwarded
  loopback port and receives the device token in the initialization result,
  which it stores in secure storage; the Host records the device as `owner` of
  that Host.
